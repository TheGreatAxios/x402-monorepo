...
import { verifyEIP3009Authorization, verifyEIP3009Forwarder } from './services/eip3009';
...
const cache = runtime.redisUrl ? new RedisCache(runtime.redisUrl) : new InMemoryCache(60_000);
...
app.get('/', (c) => {
  return c.json({
    ...
    backing: { ravendb: Boolean(store), cache: runtime.redisUrl ? 'redis' : 'memory' },
  });
});
...
app.post('/verify', async (c) => {
  ...
  if (payload.type === 'eip3009') {
    ...
    if (store) {
      ...
      await session.store({ type: 'authorization', createdAt: new Date().toISOString(), mode, payload });
      await session.saveChanges();
    }
    return c.json({ valid, mode });
  } catch (e: any) {
    return c.json({ valid: false, reason: e?.message || 'bad-request' }, 400);
  }
});
...
app.post('/settle', async (c) => {
  try {
    ...
    if (!pk) return c.json({ success: false, error: 'missing-settlement-key' }, 500);

    let txHash: string | undefined;
    if (payload.type === 'eip3009') {
      const res = await settleEIP3009(
        payload.chainId === 84532 ? 'base-sepolia' : 'skale-europa-testnet',
        runtime.rpc,
        pk,
    ...
    if (store) {
      const session = store.openSession();
      await session.store({ type: 'settlement', createdAt: new Date().toISOString(), payload, txHash });
      await session.saveChanges();
    }
      ...
      networks: [
        {
          ...
          contracts: {
            ...
            eip3009Forwarder: '0x0000000000000000000000000000000000000000',
          },
          schemes: ['exact', 'upto'],
        ...
        },
        {
          ...
          contracts: {
            ...
            eip3009Forwarder: '0x7779B0d1766e6305E5f8081E3C0CDF58FcA24330',
          },
          schemes: ['exact', 'upto'],
          methods: ['eip3009', 'eip3009-forwarder'],
        },
...
app.onError((err, c) => {
  ...
  return c.json({ success: false, error: (err as any).message ?? 'internal-error' }, 500);
});
