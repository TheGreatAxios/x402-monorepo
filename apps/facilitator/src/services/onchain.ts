...
export function getChainAndUrl(network: SupportedNetwork, rpc: RpcConfig) {
  ...
  if (network === 'skale-europa-testnet') {
    return { chain: skaleEuropaTestnet, url: rpc.skaleEuropaTestnetUrl } as const;
  }
  throw new Error(`Unsupported network: ${network}`);
}

export function getClients(network: SupportedNetwork, rpc: RpcConfig, privateKey: string) {
  const { chain, url } = getChainAndUrl(network, rpc);
  const transport = url ? http(url) : http();
  const publicClient = createPublicClient({ chain, transport });
  const walletClient = createWalletClient({ chain, transport, account: privateKeyToAccount(privateKey as Address) });
...
export async function settleEIP3009(
  ...
  const contract = getContract({ abi: eip3009Abi, address: verifyingContract, client: { public: publicClient, wallet: walletClient } as any });
  const { request } = await (contract as any).simulate.transferWithAuthorization([
    args.from,
    args.to,
    args.value,
    args.validAfter,
...
export async function settleEIP3009Forwarder(
  ...
  const { publicClient, walletClient } = getClients(network, rpc, privateKey);
  const contract = getContract({ abi: eip3009ForwarderAbi, address: forwarder, client: { public: publicClient, wallet: walletClient } as any });
