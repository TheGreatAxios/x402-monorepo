...
export async function x402Middleware(c: Context, next: Next) {
	...
	try {
		...
		if (!channel) {
			return c.json(
				{ success: false, error: "Payment channel not found" },
				404
			);
		}
		...
		if (now > channel.expiresAt) {
			return c.json(
				{ success: false, error: "Payment channel has expired" },
				400
			);
		}
		...
		if (validatedPayment.nonce <= channel.nonce) {
			return c.json(
				{ success: false, error: "Invalid payment nonce" },
				400
			);
		}
		...
		if (totalWithdrawn > channel.balance) {
			return c.json(
				{ success: false, error: "Insufficient channel balance" },
				402
			);
		}
