...
const USDC_TOKEN_ADDRESS =
	"0x9eAb55199f4481eCD7659540A17Af618766b07C4" as const;
...
async function main() {
	...
	if (!privateKey || !isHex(privateKey)) {
		throw new Error(
			"PRIVATE_KEY environment variable is required and must be a valid hex string"
		);
	}
	...
	const response = await fetch(
		"http://localhost:4021/weather?lat=32.7&lng=-96.8"
	);
		...
		if (extra.method === "eip3009-forwarder") {
			...
			if (retryResponse.ok) {
				...
				if (settleResponse.ok) {
					const settlementResult = (await settleResponse.json()) as {
						success: boolean;
						txHash?: string;
						error?: string;
					};
